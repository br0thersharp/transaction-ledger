...
impl<S: TxStore> Processor<S> {
    pub fn new(store: S) -> Self {
        Self { state: EngineState::new(store), metrics: Metrics::default() }
    }
        ...
        match tx.kind {
            TransactionType::Deposit => {
                let amount = match tx.amount {
                    Some(a) => a,
                    None => { self.metrics.missing_amount += 1; return; }
                };
            ...
            TransactionType::Dispute => {
                let rec = match store.get_mut(tx.tx) {
                    Some(r) => r,
                    None => { self.metrics.tx_not_found += 1; return; }
                };
    ...
    pub fn results(&self) -> Vec<AccountRow> {
        let mut rows: Vec<_> = self.state.accounts.iter().map(|(&client, acc)| {
            AccountRow {
                client,
